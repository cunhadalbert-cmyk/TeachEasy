(() => {
  ...
  function openAdapter(activity) {
    ...
    dialog.querySelector('#accessibility-adapter-title').textContent = `Adaptar: ${activity.topic}`;
  ...
  function questionListsFor(shell) {
    const studentArea = shell.querySelector('.collection-student-page')
      ...
      || shell.querySelector('.worksheet-page')
      || shell;
    const lists = [...shell.querySelectorAll('ol')]
  ...
  if (adaptedFilter) {
    ...
    if (label) label.lastChild.textContent = ' Acessibilidade e adaptação';
  }
