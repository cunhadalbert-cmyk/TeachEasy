(() => {
  ...
  function ensureDialog() {
    ...
    adapterDialog.innerHTML = `
      <form class="accessibility-adapter-form">
        ...
        <p class="accessibility-adapter-intro">Escolha recursos de apresentação, organização e resposta para esta atividade. O conteúdo, o gabarito e a BNCC permanecem os mesmos.</p>
        <fieldset>
          <legend>Recursos para esta versão</legend>
          <label><input type="checkbox" name="visualSupport"> <span><strong>Destacar apoio visual existente</strong><small>Valoriza imagens e figuras pedagógicas que já pertencem à atividade.</small></span></label>
          ...
          <label><input type="checkbox" name="spacious"> <span><strong>Mais espaço entre questões</strong><small>Reduz a concentração visual de informações na página.</small></span></label>
          <label><input type="checkbox" name="alternativeResponse"> <span><strong>Formas alternativas de resposta</strong><small>Quando a proposta permitir, orienta escrita, marcação, desenho, apontar ou símbolos.</small></span></label>
        ...
        <p class="accessibility-adapter-note">A adaptação muda a forma de acesso à atividade; não reduz nem substitui o objetivo pedagógico.</p>
    ...
    adapterDialog.querySelector('form')?.addEventListener('submit', event => {
      ...
      const options = Object.fromEntries(OPTION_NAMES.map(name => [
        name,
        Boolean(adapterDialog.querySelector(`[name="${name}"]`)?.checked)
      ]));
  ...
  function openAdapter(activity) {
    ...
    if (title) title.textContent = `Adaptar: ${activity.topic}`;

    OPTION_NAMES.forEach(name => {
      const input = dialog.querySelector(`[name="${name}"]`);
      if (input) input.checked = Boolean(options[name]);
    });
  ...
  function questionListsFor(shell) {
    const studentArea = shell.querySelector('.collection-student-page')
      ...
      || shell.querySelector('.early-childhood-preview')
      || shell;
  ...
  function createChecklist() {
    ...
    checklist.setAttribute('aria-label', 'Checklist da atividade');
    checklist.innerHTML = `
  ...
  function enhanceCard(article, activity) {
    ...
    button.setAttribute('aria-label', `Adaptar atividade: ${activity.topic}`);
  ...
  if (typeof renderAutismFeaturedActivities === 'function') {
    ...
    renderAutismFeaturedActivities = function renderAutismFeaturedWithAccessibility() {
      ...
      document.querySelectorAll('#autism-featured-grid .activity-library-card').forEach(card => {
        const activity = typeof activities !== 'undefined'
          ? activities.find(item => item.id === card.dataset.activityId)
          : null;
  ...
  if (typeof renderAutismFeaturedActivities === 'function'
      && typeof autismCategory !== 'undefined'
      && autismCategory) {
