...
create trigger on_teacheasy_auth_user_created
after insert on auth.users
for each row execute procedure public.handle_new_teacheasy_user();
...
begin
  ...
  if v_profile.ai_used >= v_profile.ai_limit then
  ...
  return query select
    v_profile.ai_limit,
    v_profile.ai_used,
    greatest(0, v_profile.ai_limit - v_profile.ai_used),
    v_profile.period_start,
    v_profile.period_end;
end;
$$;
