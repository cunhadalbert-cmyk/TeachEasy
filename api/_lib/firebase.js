...
function setSessionCookies(response, session) {
  ...
  response.setHeader('Set-Cookie', [
    cookie(ACCESS_COOKIE, session.idToken, Math.max(60, Number(session.expiresIn) || 3600)),
...
async function signIn(email, password) {
  return firebaseAuth('accounts:signInWithPassword', { email, password, returnSecureToken: true });
}
...
async function createProfile(userId, displayName, email) {
  ...
  const url = `${firestoreBase()}/profiles?documentId=${encodeURIComponent(userId)}`;
  const { response } = await firestoreFetch(url, { method: 'POST', body: JSON.stringify({ fields: fieldsFromObject(profile) }) });
...
function normalizeProfile(values) {
  ...
  return {
    display_name: values.displayName || '', plan: values.plan || 'premium', subscription_status: values.subscriptionStatus || 'pending_payment',
    subscription_price: Number(values.subscriptionPrice) || 19.90, mercadopago_subscription_id: values.mercadoPagoSubscriptionId || '', mercadopago_status: values.mercadoPagoStatus || '',
    ai_limit: Number(values.aiLimit) || 60, ai_used: Number(values.aiUsed) || 0, period_start: values.periodStart || '', period_end: values.periodEnd || ''
  };
}

async function getProfile(userId) {
  const doc = await readProfileDocument(userId);
  return normalizeProfile(doc?.values);
}
...
async function patchProfile(userId, patch) {
  ...
  const url = `${firestoreBase()}/profiles/${encodeURIComponent(userId)}?${fieldPaths}`;
  const { response } = await firestoreFetch(url, { method: 'PATCH', body: JSON.stringify({ fields: fieldsFromObject(patch) }) });
...
async function saveMercadoPagoSubscription(userId, subscriptionId, mercadoPagoStatus = 'pending') {
  await patchProfile(userId, { mercadoPagoSubscriptionId: String(subscriptionId), mercadoPagoStatus: String(mercadoPagoStatus || 'pending'), updatedAt: new Date().toISOString() });
}
...
async function syncSubscriptionStatus({ userId, subscriptionId, mercadoPagoStatus, nextPaymentDate }) {
  ...
  if (subscriptionStatus === 'active') {
    ...
    patch.periodEnd = nextPaymentDate || new Date(Date.now() + 31 * 24 * 60 * 60 * 1000).toISOString();
    patch.aiUsed = 0;
  }
...
async function consumeAiGeneration(userId) {
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (values.subscriptionStatus !== 'active') throw new Error('SUBSCRIPTION_INACTIVE');
    const now = new Date();
    if (!values.periodEnd || now >= new Date(values.periodEnd)) {
      const end = new Date(now); end.setMonth(end.getMonth() + 1);
      values.aiUsed = 0; values.periodStart = now.toISOString(); values.periodEnd = end.toISOString();
    }
    const limit = Number(values.aiLimit) || 60;
    const used = Number(values.aiUsed) || 0;
    if (used >= limit) throw new Error('AI_QUOTA_EXCEEDED');
...
module.exports = { clearSessionCookies, consumeAiGeneration, getAuthenticatedUser, getProfile, refundAiGeneration, saveMercadoPagoSubscription, setSessionCookies, signIn, signUp, syncSubscriptionStatus };
